...
contract("CW20Handler - [setResourceIDAndContractAddress]", async () => {
  ...
  beforeEach(async () => {
    ...
    initialResourceIDs = [
      Ethers.utils.hexZeroPad(ERC20MintableInstance1.address + Ethers.utils.hexlify(domainID).substr(2), 32),
    ];
  ...
  it("[sanity] ERC20MintableInstance1's resourceID and contract address should be set correctly", async () => {
    const retrievedTokenAddress = await ERC20HandlerInstance._resourceIDToTokenContractAddress.call(
      initialResourceIDs[0]
    );
    assert.strictEqual(Ethers.utils.getAddress(ERC20MintableInstance1.address), retrievedTokenAddress);

    const retrievedResourceID = await ERC20HandlerInstance._tokenContractAddressToResourceID.call(
  ...
  it("existing resourceID should be updated correctly with new handler address", async () => {
    ...
    const secondERC20ResourceID = [
      Ethers.utils.hexZeroPad(ERC20MintableInstance2.address + Ethers.utils.hexlify(domainID).substr(2), 32),
    ];
  ...
  it("existing resourceID should be replaced by new resourceID in handler", async () => {
    ...
    const retrievedContractAddress = await ERC20HandlerInstance._resourceIDToTokenContractAddress.call(
      secondERC20ResourceID
    );
